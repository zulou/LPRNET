...
class LPRNet:
    ...
    def _build(self):
        ...
        x = self.basic_block(x.shape[-1], 256)(x)  # Usamos x.shape[-1] para obtener el número de canales
        x = self.convolution_block(x.shape[-1], 256, 2)(x)  # Usamos x.shape[-1] para obtener el número de canales
